...
- **Curve-hungry routing.** BRouter runs on-device against `.rd5` routing tiles
  ...
  an absurd detour.
- **A cockpit, not an app.** Maximum-contrast day and night themes, a maneuver
  arrow 104 dp tall, distance in 56 sp, and every control at least 84 × 84 dp so
  it can be hit with winter gloves on a bumpy road.
...
- **No place search.** Destinations are set by tapping the map. Offline
  geocoding would need a place index the rider does not have, and the app has no
...
the Kalman filter, map matching, the turn-by-turn state machine — is written
without Android imports, so it can be compiled and tested on a plain JVM:
